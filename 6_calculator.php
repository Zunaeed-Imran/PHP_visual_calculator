<!-- // CALCULATOR WITHOUT Try catch  function -->

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
</head>
<body>
    <div>
        <input type="text" class="display" id="display"><br>
        <button class="button" onclick="display('9')">9</button>
        <button class="button" onclick="display('8')">8</button>
        <button class="button" onclick="display('7')">7</button><br>
        <button class="button" onclick="display('6')">6</button>
        <button class="button" onclick="display('5')">5</button>
        <button class="button" onclick="display('4')">4</button><br>
        <button class="button" onclick="display('3')">3</button>
        <button class="button" onclick="display('2')">2</button>
        <button class="button" onclick="display('1')">1</button><br>
        <button class="button" onclick="cle('c')">C</button>
        <button class="button" onclick="display('0')">0</button>
        <button class="button" onclick="calculate()">=</button><br>
        <button class="button" onclick="display('+')">+</button>
        <button class="button" onclick="display('-')">-</button>
        <button class="button" onclick="display('*')">*</button><br>
        <button class="button" onclick="display('/')">/</button>
    </div>



    <script>

        function display(val){
            document.getElementById('display').value += val;
        }

        function cle(){
            document.getElementById('display').value = '';
        }

        function calculate(){
            const result = eval(document.getElementById('display').value); // we made it without try catch using.
            document.getElementById('display').value = result;
        }

    </script>
</body>
</html>